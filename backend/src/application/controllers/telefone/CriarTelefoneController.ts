...
import { CriarTelefoneInput } from '../../../domain/implementations/useCases/telefone/criarTelefone/CriarTelefoneInput';
...
export class CriarTelefoneController implements IController {
  constructor(private useCase: CriarTelefone) { }
  ...
  public async execute(pData: EntrypointData): Promise<EntryPointSuccess> {
    ...
    try {
      ...
      const inputTelefone = new CriarTelefoneInput({
        body: pData.body,
        parametros: pData.parametros,
        tokenAuthorization: pData.tokenAuthorization,
      });
      ...
      if (result) {
        return new EntryPointSuccess('Telefone criado com sucesso.', result);
      }
      return new EntryPointResponse(false, 400, 'Pessoa não encontrada, ou telefone já cadastrado para a pessoa.', result, null);
    } catch (error) {
